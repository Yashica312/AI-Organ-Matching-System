...
def prepare_training_data(donors_df):
    ...
    train_df["organ_type"] = train_df["donor_organ"].fillna("Kidney")
    train_df["dataset_source"] = "live"
    train_df["compatibility_score"] = train_df.apply(
        lambda row: compatibility_score(row["donor_bg"], "A"),
        axis=1,
    )
...
def render_registry_tables(donors_df, recipients_df):
...
selected_index = st.selectbox(
    ...
    format_func=lambda idx: (
        f"{recipients_df.loc[idx, 'recipient_name']} - {recipients_df.loc[idx, 'required_organ']}"
    ),
)
...
pairs["organ_type"] = selected["required_organ"]
pairs["dataset_source"] = "live"
pairs["compatibility_score"] = pairs.apply(
    lambda row: compatibility_score(row["donor_bg"], selected["recipient_bg"]),
    axis=1,
)
pairs = pairs[pairs["donor_organ"] == selected["required_organ"]].copy()
...
best_col3.metric("Final Score", round(top["final_score"], 3))

best_col4, best_col5 = st.columns(2)
best_col4.metric("Donor Age", int(top["donor_age"]))
best_col5.metric("Compatibility", round(top["compatibility_score"], 2))
